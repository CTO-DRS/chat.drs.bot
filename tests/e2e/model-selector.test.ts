...
test.describe("Model Selector", () => {
  ...
  test("shows available models", async ({ page }) => {
    ...
    await expect(
      availableModels.getByRole("option", { name: "GLM-4.6" })
    ).toBeVisible();
    await expect(
      availableModels.getByRole("option", { name: "GLM-4 Flash" })
    ).toBeVisible();
  });
  ...
  test("can select a different model", async ({ page }) => {
    ...
    await page.getByRole("option", { name: "GLM-4.6" }).click();

    await expect(page.getByPlaceholder("Search models...")).not.toBeVisible();
    await expect(modelButton).toContainText("GLM-4.6");
  });
});
